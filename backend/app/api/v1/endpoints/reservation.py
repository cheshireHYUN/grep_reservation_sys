...
@router.get("/available-times", response_model=APIResponse[List[TimeSlotResponseSchema]], summary="예약 가능한 시간 조회", description="3일 이후의 예약가능 시간을 조회합니다.")
def get_available_times_api(db: Session = Depends(get_db)):
    available_times = reservation_service.get_available_times(db)

    if not available_times:
        raise APIException("예약 가능한 시간이 존재하지 않습니다.", status_code=404)
    
    return success_response(available_times)
...
@router.get("", response_model=APIResponse[List[ReservationResponseSchema]], summary="예약한 목록 조회", description="로그인한 유저의 예약목록을 신청순으로 조회합니다.")
