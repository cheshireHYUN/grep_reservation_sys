...
@router.get("", response_model=APIResponse[List[ReservationResponseSchema]], summary="전체 예약 리스트 조회", description="전체 예약 리스트를 생성순서대로 조회합니다.")
def get_all_reservations_for_admin(db: Session = Depends(get_db)):
    return success_response(admin_service.get_all_reservations_for_admin(db))
...
@router.patch("/{reservation_id}", summary="예약 수정", response_model=APIResponse[None], description="인원수나 날짜를 입력해 예약을 수정합니다. 확정된 예약은 수정이 불가능합니다.")
